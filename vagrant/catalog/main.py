...
APPLICATION_NAME = "Items Catalogue Application"
...
def showItems(category_id):
    ...
    json_catergory = category.serialize

    return render_template('items.html', items=items,
                           categories=categories,
                           category=category,
                           category_se=json_catergory)
...
def categoryJson():
    ...
    """
    categories = session.query(Category).all()
    catergoriesSerielized = [i.serialize for i in categories]
    return jsonify(Category=catergoriesSerielized)


@app.route('/catalogue/<category_id>/<item_id>/JSON/')
def itemJson(item_id, category_id=None):
